...
DWT-RNN Classifier
==================
DWT 에너지 특징 (level+1, n_pca) 형태를 RNN으로 분류합니다.

특징 → reshape (feature_dim,) → (level+1, n_pca)
    → StandardScaler → RNN → 마지막 은닉 상태 → MLP classifier

실행 예시
---------
  python dwt_rnn.py --feat_dir ../../data/feature_extraction/dwt/260406
"""
...
def load_from_feature_npz(
    ...
    """
    DWT energy feature NPZ 로드 후 reshape.

    Returns
    -------
    X        : (N, level+1, n_pca_used) float32
    y        : (N,) str
    subjects : (N,) str
    """
    ...
    for fp in files:
        ...
        feat = d["features"].astype(np.float32)  # (feature_dim,)
        feature_dim = len(feat)
        n_pca_used = feature_dim // (level + 1)

        # Reshape: (feature_dim,) → (level+1, n_pca_used)
        feat_reshaped = feat.reshape(level + 1, n_pca_used)
        X.append(feat_reshaped)
        y.append(label)
        subjects.append(str(d["subject"]))

    X = np.array(X, dtype=np.float32)  # (N, level+1, n_pca_used)
    return X, np.array(y), np.array(subjects)


# ══════════════════════════════════════════════════════════════
# B. RNN 모델
# ══════════════════════════════════════════════════════════════
class DWTRNNClassifier(nn.Module):
    def __init__(
        self,
        input_size: int,
        hidden_size: int = 128,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x: (batch, seq_len=level+1, input_size=n_pca)
        out, h_n = self.rnn(x)
...
def train_and_evaluate(X, y, subjects, args):
    ...
    # ── 학습
    print(f"\n[Train] Training RNN (hidden={hidden_size}, layers={num_layers}, dropout={dropout})")
    batch_size = 64
    epochs = args.epochs
    best_loss = float("inf")
    patience = 20
    ...
    for epoch in range(epochs):
        ...
        for i in range(0, len(X_tr_scaled), batch_size):
            X_batch = torch.tensor(X_tr_scaled[i:i+batch_size], dtype=torch.float32).to(device)
            y_batch = torch.tensor(y_tr[i:i+batch_size], dtype=torch.long).to(device)
    ...
    if overwrite:
        report_fname = "dwt_rnn_report.txt"
        confusion_tag = ""
    else:
        report_fname  = f"dwt_rnn_report_{ts}.txt"
        confusion_tag = ts

    report_path = os.path.join(args.result_dir, report_fname)
    with open(report_path, "w", encoding="utf-8") as f:
        f.write(f"DWT-RNN Classification Report\n")
        f.write(f"Timestamp : {ts}\n")
        f.write(f"Feature   : DWT (level={args.level}, n_pca={args.n_pca})\n")
    ...
    for fold, (tr_idx, val_idx) in enumerate(kf.split(X, y_enc)):
        ...
        for epoch in range(args.epochs):
            ...
            for i in range(0, len(X_tr_fold_scaled), batch_size):
                X_b = torch.tensor(X_tr_fold_scaled[i:i+batch_size], dtype=torch.float32).to(device)
                y_b = torch.tensor(y_tr_fold[i:i+batch_size], dtype=torch.long).to(device)
                opt_fold.zero_grad()
                logits = model_fold(X_b)
        ...
        model_fold.eval()
        with torch.no_grad():
            val_logits = model_fold(torch.tensor(X_val_fold_scaled, dtype=torch.float32).to(device))
    ...
    # ── 모델 저장
    model_path   = os.path.join(args.model_dir, "dwt_rnn.pth")
    scaler_path  = os.path.join(args.model_dir, "dwt_rnn_scaler.pkl")
    le_path      = os.path.join(args.model_dir, "dwt_rnn_label_encoder.pkl")
    ...
    meta = {
        ...
        "seq_len": int(args.level + 1),
        "test_accuracy": float(acc),
        "cv_mean": float(cv_mean),
        "cv_std": float(cv_std),
        "timestamp": ts,
    }
    meta_path = os.path.join(args.model_dir, "dwt_rnn_meta.json")
...
def _plot_confusion(cm, classes, ts, result_dir):
    ...
    ax.set_xticklabels(classes, color=TEXT)
    ...
    ax.set_title("DWT-RNN Confusion Matrix", color=TEXT, fontsize=12, pad=8)
    ...
    plt.tight_layout()
    fname = "dwt_rnn_confusion.png" if not ts else f"dwt_rnn_confusion_{ts}.png"
...
# ══════════════════════════════════════════════════════════════
def _parse_args():
    p = argparse.ArgumentParser(description="DWT-RNN Classifier for WiFi CSI stride classification")
...
def main():
    args = _parse_args()
    X, y, subjects = load_from_feature_npz(args.feat_dir, args.level, args.n_pca)
    if len(X) == 0:
