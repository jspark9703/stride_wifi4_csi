"""
DWT-seq LSTM Classifier
=======================
DWT 노이즈 제거 시퀀스 (가변 길이)를 LSTM으로 분류합니다.

시퀀스 → zero-pad to max_len → pack_padded_sequence → LSTM → last hidden → MLP

실행 예시
---------
  python dwt_seq_lstm.py --feat_dir ../../data/feature_extraction/dwt-seq/260406
"""
...
# ══════════════════════════════════════════════════════════════
# B. LSTM 모델 (pack_padded_sequence 지원)
# ══════════════════════════════════════════════════════════════
class DWTSeqLSTMClassifier(nn.Module):
    def __init__(
        self,
        input_size: int,
        hidden_size: int = 128,
...
def train_and_evaluate(X, y, subjects, args):
    ...
    # ── 학습
    print(f"\n[Train] Training DWT-seq LSTM (hidden={hidden_size}, layers={num_layers}, max_seq_len={max_seq_len})")
    batch_size = 64
    epochs = args.epochs
    best_loss = float("inf")
    patience = 20
    ...
    if overwrite:
        report_fname = "dwt_seq_lstm_report.txt"
        confusion_tag = ""
    else:
        report_fname  = f"dwt_seq_lstm_report_{ts}.txt"
        confusion_tag = ts

    report_path = os.path.join(args.result_dir, report_fname)
    with open(report_path, "w", encoding="utf-8") as f:
        f.write(f"DWT-seq LSTM Classification Report\n")
    ...
    for fold, (tr_idx, val_idx) in enumerate(kf.split(X, y_enc)):
        ...
        model_fold = DWTSeqLSTMClassifier(
    ...
    # ── 모델 저장
    model_path   = os.path.join(args.model_dir, "dwt_seq_lstm.pth")
    scaler_path  = os.path.join(args.model_dir, "dwt_seq_lstm_scaler.pkl")
    le_path      = os.path.join(args.model_dir, "dwt_seq_lstm_label_encoder.pkl")
    ...
    meta_path = os.path.join(args.model_dir, "dwt_seq_lstm_meta.json")
...
def _plot_confusion(cm, classes, ts, result_dir):
    ...
    ax.set_ylabel("True")
    ax.set_title("DWT-seq LSTM Confusion Matrix", fontsize=12, pad=8)
    ...
    plt.tight_layout()
    fname = "dwt_seq_lstm_confusion.png" if not ts else f"dwt_seq_lstm_confusion_{ts}.png"
...
# ══════════════════════════════════════════════════════════════
def _parse_args():
    p = argparse.ArgumentParser(description="DWT-seq LSTM Classifier for WiFi CSI stride classification")
    ...
    p.add_argument("--lstm_hidden", type=int, default=128, help="LSTM hidden size (default: 128)")
    p.add_argument("--lstm_layers", type=int, default=2,   help="LSTM num layers (default: 2)")
    p.add_argument("--lstm_dropout", type=float, default=0.3, help="LSTM dropout (default: 0.3)")
